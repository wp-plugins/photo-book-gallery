		...
		if (isset($allbooks['books'])) {
			foreach ($allbooks['books'] as $key => $data) {
				...
				add_shortcode( $full_shortcode, function() use ($data){
					...
					wp_localize_script( 'wcp-custom-script', 'book', array( 'width' => $data['width'], 'height' => $data['height'], 'delay' => $data['autodelay']));
					...
					<?php foreach ($allbooks['books'] as $key => $data) { ?>
			  		...
			  		<div class="tab-content">
						...
						<table class="form-table">
							<tr>
								<td>Book Width: </td>
								<td><input class="bookwidth" type="text" value="<?php echo $data['width']; ?>"></td>
								<td>Book Height</td>
								<td><input class="bookheight" type="text" value="<?php echo $data['height']; ?>"></td>
								<td>AutoPlay delay for each page in ms (leave blank to disable autoplay)</td>
								<td><input class="bookautoplaydelay" type="number" value="<?php echo $data['autodelay']; ?>"></td>
							</tr>
						</table>
						<hr>
					...
					<h3 class="tab-head">Photo Book</h3>
			  		<div class="tab-content">
						...
						<table class="form-table">
							<tr>
								<td>Book Width: </td>
								<td><input class="bookwidth" type="text" value="450"></td>
								<td>Book Height</td>
								<td><input class="bookheight" type="text" value="450"></td>
								<td>AutoPlay delay for each page in ms (leave blank to disable autoplay)</td>
								<td><input class="bookautoplaydelay" type="number" value=""></td>
							</tr>
						</table>
						<hr>
